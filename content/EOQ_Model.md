...
库存管理本质上是确定合适的产品在合适的位置以合适的形式的数量。战略决策包括产品层面和网络设计的影响。战术决策包括部署和确定携带什么物品，以什么形式（原材料、在制品、成品等），以及在哪里。最后，运营决策决定了这些库存的补货政策（何时和多少），寻求能使这些总成本最小化的补货策略。补货策略本质上需要说明两件事：要订购的数量，以及何时订购。

不同的补货策略模型对应不同的需求和供应的假设（需求是常量还是变量，确定还是随机等）。这里首先从介绍 EOQ Model。
<!-- more -->
...
既然EOQ 模型对订购量Q的变化非常稳定，那么如果考虑最初的假设，使用的时需求（D），如果使用预测（F）来替代对TRC会有多大的影响呢？尤其是考虑到预测往往会和实际需求有偏差。
...
通过上述分析可以看到，EOQ模型有非常严格的假设要求，但是仍然有非常广泛的用途。当存在长期且稳定的需求时，EOQ模型都是一个很好的开始，可以根据实际情况调整Q。EOQ模型平衡的是库存持有成本和订购成本，当有其它的考虑因素时，可以知道即使Q偏离\\(Q^\ast\\)，那么也可以预估TRC的变化幅度。另外一点需要注意的是TRC只是TC的一部分，那么TC的变化幅度则会更小。
...
根据商务条款的不同，在途库存有可能会需要纳入考虑（比如EXW），这里假设在途库存的 Holding cost也需要考虑。那么新的TC方程可以表示为：
 $$ TC(Q) = cD + c_t\frac D Q + c_e(\frac Q 2 + DL)+ c_sE[UnitsShort] $$

根据前面的推导，可以得出，Lead Time(L) > 0时，最佳订购量\\(Q^\ast\\)不会发生变化。真正会影响的是补货的时间点，为了避免缺货，L>0时的补货策略更新为：
- 当库存低于DL时，预定 \\(Q^\ast \\)
- 每隔 \\(T^\ast \\) 的时间预定 \\(Q^\ast \\)
